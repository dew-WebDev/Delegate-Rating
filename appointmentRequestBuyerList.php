...
<title>Appointment&nbsp;Request&nbsp;Buyer&nbsp;List</title>
...
if(isset($_SESSION['mas_user_name']) && $_SESSION['mas_user_name'] == 'Y')
{
	echo "<ul class='nav navbar-nav side-nav'>";
		...
		echo "<li class='active'><a href='viewAppointmentRequestBuyer.php'><i class='fa fa-briefcase'></i>View Appointment Requests</a></li>";
...
<?php
	...
	if(mysql_num_rows($tq) > 0 && $rs=mysql_fetch_array($tq))
	{
		...
		if($vappon == 'N')
		{
			echo "<li class='active'><a href='confirmAppointmentRequestBuyer.php'><i class='fa fa-briefcase'></i>&nbsp;View Appointment Requests</a></li>";
		}
		else
		{
			$b=false;
			...
			if($b)
			{
				echo "<li class='active'><a href='confirmAppointmentRequestBuyer.php'><i class='fa fa-briefcase'></i>
		View Appointment Requests</a></li>";
			}
			else
			{
				echo "<li class='active'><a href='viewAppointmentRequestBuyer.php'><i class='fa fa-briefcase'></i>
		View Appointment Requests</a></li>";
			}
		}
	}
...
<input type='hidden' id='hidid' name='hidid' value='<?php if(isset($_GET['id'])) { echo $_GET['id']; } ?>' />
<table	class="table table-bordered table-hover table-striped tablesorter">
	...
	<tbody>
		...
		<tr>
			<td>&nbsp;</td>
			<td>
				<input type='button' class='btn btn-default' style='width:250px;' id='searchBut' value='Search' onclick="advanceBuyerList('2');">
				...
				<div class="panel-body">
					...
					<input type='hidden' id='svalue' name='svalue' value="like '%'"/>
						...
						<?php
							...
							echo "<a href='#' onClick=\"$('#svalue').val('regexp \'^[0-9]\'');$('#index').html('0-9');advanceBuyerList('2');\">0-9</a>&nbsp;&nbsp;|&nbsp;&nbsp;";
							for($i=65;$i<=90;$i++)
							{
								echo "<a href='#' onClick=\"$('#svalue').val('like \'".chr($i)."%\'');$('#index').html('".chr($i)."');advanceBuyerList('2');\">".chr($i)."</a>&nbsp;&nbsp;|&nbsp;&nbsp;";
							}
							echo "<a href='#' onClick=\"$('#svalue').val('like \'%\'');$('#index').html('A-Z');advanceBuyerList('2');\">ALL</a>&nbsp;&nbsp;</span>";
						...
						<span>
							<center>
								<input type='button' class='btn btn-default' style='width:150px;' id='butsave' value='Save' onClick="location.replace('viewAppointmentRequestBuyer.php');" />
								<input type='button' class='btn btn-default' style='width:250px;' id='butview' value='View My Appoinment Requests' onClick="location.replace('viewAppointmentRequestBuyer.php');" />
							</center>
						</span>
					</div>
				</div>
...
if (isset($_GET['msg']))
{
	echo "<input type='hidden' id='hidmsg' value='".$_GET['msg']."'>";
}
else
{
	echo "<input type='hidden' id='hidmsg' value=''>";
}
?>
<iframe style='display:none;' id='hidframe' name='hidframe'></iframe>
<iframe style='display:none;' id='hidframe1' name='hidframe1'></iframe>
...
<script lang="javascript">
	...
	if ($("#hidmsg").val() == 'max')
	{
		alert("Please note, You have reached your maximum limit of appointment requests.");
	}
	function loadData()
	{
		$("#selcompany").val("");
