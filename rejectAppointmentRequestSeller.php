...
<title>Reject&nbsp;Appointment&nbsp;Request&nbsp;Seller</title>
...
$edateo = mysql_query("select* from emaildate where emaildate_status = 'Y'");
	if(mysql_num_rows($edateo)>0)
	{
		if($edateoo=mysql_fetch_array($edateo))
		{
			...
			$rejection_msg_date=$edateoo['rejection_msg_date'];
		}
	}
...
if(isset($_SESSION['mas_user_name']) && $_SESSION['mas_user_name'] == 'Y')
{
	...
	echo "<li class='active'><a href='rejectAppointmentRequestSeller.php'><i class='fa fa-thumbs-o-down'></i>&nbsp;Rejection of unwanted appointment requests<br>
...
<h3 class="panel-title">REJECTION OF UNWANTED APPOINTMENT REQUESTS</h3>
</div>
<div class="panel-body">
<div class="panel-body">
<center><span style='font-size:16px;'><b><?php echo $_SESSION['comname']; ?></b></span></center>
	<?php
	if($_SESSION['chk'] == '1')
	{
		echo "<center><p style='font-size:16px;text-align:justify;'>&nbsp;&nbsp;Please view below appointment requests from sellers. If you do not want PATA to match any requests, please reject them. You are allowed to reject maximum 5 requests. Anyhow, there is no action required on this page if you do not want to reject any requests. To view your Business Calendar, please log in this system on ".$rejection_msg_date.".</p></center>";
	}
	else
	{
		echo "<center><p style='font-size:16px;text-align:justify;'>&nbsp;&nbsp;Please view below appointment requests from sellers. If you do not want PATA to match any requests, please reject them. You are allowed to reject maximum 10 requests. Anyhow, there is no action required on this page if you do not want to reject any requests. To view your Business Calendar, please log in this system on ".$rejection_msg_date.".</p></center>";
	}
?>
...
<script src="http://cdn.oesmith.co.uk/morris-0.4.3.min.js"></script> <script
	...
	{
		ajaxfunction("query/rejectAppointmentRequestBuyer.php", "document.getElementById('ajaxResult').innerHTML = ajaxRequest.responseText;initTableSorter();");
	}
</script>
</body>
</html>