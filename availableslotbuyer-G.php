...
if(isset($_SESSION['mas_user_name']) && $_SESSION['mas_user_name'] == 'Y')
{
	...
	echo"<li><a href='business-calendarbuyerconfirm.php'><i class='fa fa-book'></i>&nbsp;Business Calendar</a></li>
		<li class='active'><a href='availableslotbuyer.php'><i class='fa fa-clock-o'></i>&nbsp;Available Slot</a></li>
				...
				<div class="col-lg-12">
					<div class="panel panel-primary">
						<div class="panel-heading">
							<h3 class="panel-title">SELLER AVAILABLE SLOTS</h3>
						</div>
						<div class="panel-body">
							<div class="panel-body">
								<div class="table-responsive">
                <form action="availableslotbuyer.php" method="get" accept-charset="utf-8" enctype="multipart/form-data">
									<table
...
<thead>
	<tr>
		...
		<th style='text-align:center;'>Seller Company Name&nbsp;<i class="fa fa-sort"></i></th>
		<th style='text-align:center;'>Delegate Name&nbsp;<i class="fa fa-sort"></i></th>
		<th style='text-align:center;'>Country&nbsp;<i class="fa fa-sort"></i></th>
		<th style='text-align:center;'>Booth No&nbsp;<i class="fa fa-sort"></i></th>
...
seller_company.company_id as compid,
seller_company.first_time_attending as first_time_attending,
c.country_name as cntName,
buyer_company.company_name as buyer,
buyer_delegate_detail.mas_buyer_id as mbcid,
mas_seller.mas_seller_id as mscid,
mas_seller.title_name as title_name,
mas_seller.name as sellname,
mas_seller.middle_name as middle_name,
mas_seller.family_name as family_name,
seller_delegate_detail.username as usrname,
CASE
...
END as seller_type,
boothNumber,
mas_seller.email as txtemail
from 
seller_company 
left join countries c on c.country_id=seller_company.cnt_id
inner join buyer_company on buyer_company.buyer_com_id = ".$_SESSION['did']."
...
inner join buyer_delegate_detail on buyer_delegate_detail.mas_buyer_id = mas_buyer.mas_buyer_id
...
, time_slots
where
buyer_company.buyer_com_id = ".$_SESSION['did']."
and
...
and mas_buyer_status = 'Y'
and (mas_buyer.title_name is not null and mas_buyer.title_name <> 'None')
and (mas_buyer.name is not null and mas_buyer.name <> '')
and
buyer_delegate_status = 'Y'
and
seller_company.seller_status='Y' 
and 
(seller_org_details.seller_org_status='Y' or seller_org_details.seller_org_status is null) 
and
(seller_delegate_detail.seller_delegate_status='Y')
and
(buyer_delegate_detail.buyer_delegate_status='Y')
and
...
and
(seller_org_details.pre_addendum='0' or seller_org_details.seller_org_status is null) 
and
not exists 
(
...
and 
time_slot_id not in (select time_slot_id from appointments where appointments.mas_seller_com_id = mas_seller.mas_seller_id)
and
(seller_missing_fields.seller_missing_fields_status is null or seller_missing_fields.seller_missing_fields_status='Y')
and
(seller_company.seller_status is null or seller_company.seller_status='Y')
and time_slot_id = '".$_GET['slot']."'
and seller_company.company_name like '%".$_GET['selcompany']."%'";

if($_GET['selcountryse'] != 0)
{
$q.=" and seller_company.cnt_id = ".$_GET['selcountryse'];
}
$q.="
union
...
(buyer_org_details.mark_for_delete is null or buyer_org_details.mark_for_delete <> '1')
and
(seller_org_details.pre_addendum='2') 
and
not exists 
(
select time_slot_id from appointments where appointments.seller_com_id = seller_company.seller_com_id and appointments.buyer_com_id = buyer_company.buyer_com_id
...
mas_seller.mas_seller_id as mscid,
CASE
	WHEN seller_missing_fields.pavillion_type = '3' AND time_slots.time_slot_date = '".$emaildateo."' THEN time_slots.time_slot_id
	WHEN seller_missing_fields.pavillion_type = '3' AND time_slots.time_slot_date <> '".$emaildateo."' THEN '-1'
	ELSE time_slots.time_slot_id
END as time_slot_id,
CASE
	WHEN seller_missing_fields.pavillion_type = '3' AND time_slots.time_slot_date = '".$emaildateo."' THEN 'A'
	WHEN seller_missing_fields.pavillion_type = '3' AND time_slots.time_slot_date <> '".$emaildateo."' THEN 'C'
	ELSE 'B'
END as seller_type,
boothNumber
from 
		...
		if(mysql_num_rows($qry)>0)
		{
			while($rs=mysql_fetch_array($qry))
			{
				...
				if($rs['first_time_attending'] == 1)
				{
						...
						echo "<tr><td style='width:10%;'><center>".$rs['usrname']."</center></td><td style='width:50%;'><a href='#' onClick=\"redirectPage('".$rs['scid']."','2','avbu');\">".$rs['seller']."</a></td><td style='width:15%;'>".$rs['title_name']." ".$rs['sellname']." ".$rs['middle_name']." ".$rs['family_name']."</td><td style='width:15%;text-align:center;'>".$rs['cntName']."</td><td style='width:10%;text-align:center;'>".$rs['boothNumber']."</td><td style='text-align:center;width:10%;'><center><a data-toggle='modal' data-target='#myModal' href='#' onClick=\"getMailDetail('".$rs['scid']."','".$rs['txtemail']."','".$rs['seller']."');return false;\"><i class='fa fa-envelope'></i></a></center></td></tr>";
	...
	<form action="query/sendAppointmentEmail.php" id="uploadform" method="post" class="form-horizontal" role="form" accept-charset="utf-8" enctype="multipart/form-data">
      ...
      <div class="modal-body">
		...
		<div class="form-group">
			...
			<div class="col-md-11">
			<?php
			$q=mysql_query("select email from mas_buyer inner join buyer_delegate_detail on mas_buyer.mas_buyer_id = buyer_delegate_detail.mas_buyer_id and username = '".$_SESSION['user_name']."' and buyer_delegate_status = 'Y' and mas_buyer_status = 'Y'");
		...
		<div class="form-group">
			<div class="col-md-12">
				...
				<?php
				$qryn=mysql_query("select fullname from mas_buyer inner join buyer_delegate_detail on mas_buyer.mas_buyer_id = buyer_delegate_detail.mas_buyer_id and buyer_delegate_detail.username = '".$_SESSION['user_name']."' and buyer_delegate_status = 'Y' and mas_buyer_status = 'Y'");
...
<script>
	...
	{
		location.replace("sellerDetails.php?id="+id+"&fl="+val1+"&fk="+val2+"&slot="+$('#slot').val()+"&selcompany="+$('#hidcom').val()+"&selcountryse="+$('#hidcon').val());
	}
