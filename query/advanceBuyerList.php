...
session_start();
	try
	{
		...
		{
			$qry.=" and bmf.CNT_ID IN (".$_GET['selcountry'].")";
		}
		...
		if(isset($_GET['selbuyprofile']) && $_GET['selbuyprofile'] != 0)
		{
			...
			$listarrayprofile = explode(",", $_GET['selbuyprofile']);
			$qryselselprofile = "";
			for ($index = 1; $index <= 17; $index++){
	            if($qryselselprofile === ""){
					if (in_array($index,$listarrayprofile)){
		                $qryselselprofile.=" and (bmf.seek".$index."= ".$index;
		            }
				}else{
					if (in_array($index,$listarrayprofile)){
		                $qryselselprofile.="  or bmf.seek".$index."= ".$index;
		            }
				}
	        }
	        ...
	        if (in_array('00', $listarrayprofile)){
	        	if($qryselselprofile === ""){
	        		$qryselselprofile.=" and bmf.otherseek is not null ";
	        	}else{
	        		$qryselselprofile.=" or bmf.otherseek is not null ";
	        	}
		...
		if(mysql_num_rows($q)>0)
		{
			while($rs=mysql_fetch_array($q))
			{
				...
				if($rs['FirstTimeBuyer'] == 1)
				{
					if ($cntchk > -1)
					{
						if (($ptype == 3 && $cntchk < 25) || ($ptype != 3 && $cntchk < 25))  // Maximum Appointment Selections
						{
							$op.= "<tr style='width:10%;'><td>".$rs['cid']."</td><td style='width:70%;'><a href=\"buyerDetails.php?id=".$rs['bcid']."&fk=abl\">".$rs['cname']."</a></td><td style='width:20%;'>".$rs['cntName']."</td></tr>";
						}
						else
						{
							$op.= "<tr style='width:10%;'><td>".$rs['cid']."</td><td style='width:70%;'><a href=\"buyerDetails.php?id=".$rs['bcid']."&fl=1&fk=abl\">".$rs['cname']."</a></td><td style='width:20%;'>".$rs['cntName']."</td></tr>";
						}
					}
					else
					{
						$op.= "<tr style='width:10%;'><td>".$rs['cid']."</td><td style='width:70%;'><a href=\"buyerDetails.php?id=".$rs['bcid']."&fl=2&fk=abl\">".$rs['cname']."</a></td><td style='width:20%;'>".$rs['cntName']."</td></tr>";
					}
				}
				else
				{
