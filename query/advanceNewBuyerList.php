...
session_start();
	try
	{
		...
		inner join buyer_org_details bod on bc.buyer_com_id = bod.buyer_com_id and (bod.buyer_org_status is null or bod.buyer_org_status = 'Y') and (bod.mark_for_delete is null or bod.mark_for_delete = '0') and (bod.pre_addendum = '2' or bdd.pre_addendum = '2')
			...
			while($rs=mysql_fetch_array($q))
			{
				...
				if($rs['FirstTimeBuyer'] == 1)
				{
					if ($cntchk > -1)
					{
						...
						{
							$op.= "<tr style='width:10%;'><td>".$rs['cid']."</td><td style='width:70%;'><a href=\"buyerDetails.php?id=".$rs['bcid']."&fk=nbc\">".$rs['cname']."</a></td><td style='width:20%;'>".$rs['cntName']."</td></tr>";
						}
						else
						{
							$op.= "<tr style='width:10%;'><td>".$rs['cid']."</td><td style='width:70%;'><a href=\"buyerDetails.php?id=".$rs['bcid']."&fl=1&fk=nbc\">".$rs['cname']."</a></td><td style='width:20%;'>".$rs['cntName']."</td></tr>";
						}
					}
					else
					{
						$op.= "<tr style='width:10%;'><td>".$rs['cid']."</td><td style='width:70%;'><a href=\"buyerDetails.php?id=".$rs['bcid']."&fl=2&fk=nbc\">".$rs['cname']."</a></td><td style='width:20%;'>".$rs['cntName']."</td></tr>";
					}
				}
				else
				{
