...
session_start();
	try
	{
		...
		inner join seller_org_details sod on sc.seller_com_id = sod.seller_com_id and (sod.seller_org_status is null or sod.seller_org_status = 'Y') and (sod.markForDelete is null or sod.markForDelete = '0') and (sod.pre_addendum = '2' or sdd.pre_addendum = '2')
			...
			while($rs=mysql_fetch_array($q))
			{
				...
				if($rs['first_time_attending'] == 1)
				{
					if ($cntchk > -1)
					{
						...
						{
							$op.= "<tr style='width:10%;'><td>".$rs['cid']."</td><td style='width:70%;'><a href=\"sellerDetails.php?id=".$rs['scid']."&fk=nsc\">".$rs['cname']."</a></td><td style='width:20%;'>".$rs['cntName']."</td></tr>";
						}
						else
						{
							$op.= "<tr style='width:10%;'><td>".$rs['cid']."</td><td style='width:70%;'><a href=\"sellerDetails.php?id=".$rs['scid']."&fl=1&fk=nsc\">".$rs['cname']."</a></td><td style='width:20%;'>".$rs['cntName']."</td></tr>";
						}
					}
					else
					{
						$op.= "<tr style='width:10%;'><td>".$rs['cid']."</td><td style='width:70%;'><a href=\"sellerDetails.php?id=".$rs['scid']."&fl=2&fk=nsc\">".$rs['cname']."</a></td><td style='width:20%;'>".$rs['cntName']."</td></tr>";
					}
				}
				else
				{
