...
<title>Advance&nbsp;Seller&nbsp;List</title>
...
if(isset($_SESSION['mas_user_name']) && $_SESSION['mas_user_name'] == 'Y')
{
	...
	echo "<li class='active'><a href='advanceSellerList.php'><i class='fa fa-search'></i> Advance Seller List<br><font class='sidebar-alt'>".$advance_buyer_list."</font></a></li>";
...
?>
	<li class='active'><a href="advanceSellerList.php"><i
...
<?php
	...
	if(mysql_num_rows($tq) > 0 && $rs=mysql_fetch_array($tq))
	{
		...
		if($vappon == 'N')
		{
			echo "<li><a href='AppointmentRequestSeller.php'><i class='fa fa-briefcase'></i>&nbsp;View Appointment Requests</a></li>";
		}
		else
		{
			$b=false;
...
<h3 class="panel-title">ADVANCE SELLER LIST</h3>
</div>
...
<table
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<ul class="ij-options" id="selselprofilemultiple">

			    </ul>
				<select id='selselprofilemultipleselect' class='form-control' style='width:500px;display:inherit;'>
					...
					<option value='0' selected>All Seller Profile</option>
					<option value='01' >Cultural Tourism</option>
					<option value='02' >Ecotourism</option>
					<option value='03' >Wildlife Observation, Bird Watching, Safari</option>
					<option value='04' >Expedition Cruising</option>
					<option value='10' >Mountain Tourism (i.e. Climbing, Trekking, Hiking)</option>
					<option value='11' >Cycling, Motor biking</option>
					<option value='12' >Water Sports (i.e. Snorkelling, Diving, Rafting, Kayaking)</option>
					<option value='13' >Extreme Sports (i.e. Bungee Jumping, Paragliding, Parachuting, Zip-Lining, Surfing)</option>
					<option value='19' >National Tourism Organisation / Regional Tourism Organisation</option>
					<option value='20' >Inbound Adventure Tour Operator</option>
					<option value='21' >Family Adventure</option>
					<option value='22' >Accommodation</option>
					<option value='23' >Transportation</option>
					<option value='24' >Outdoor Equipment Retailers</option>
					<option value='25' >Sport Gear Retailers</option>
...
-->		
		<tr>
			...
			<td><input type='button' class='btn btn-default'
				...
				onclick="advanceSellerList_25012017();"> <input type='button'
...
<div id='sort'><?php
							...
							echo "<a href='#' onClick=\"$('#svalue').val('regexp \'^[0-9]\'');$('#index').html('0-9');advanceSellerList_25012017();\">0-9</a>&nbsp;&nbsp;|&nbsp;&nbsp;";
							for($i=65;$i<=90;$i++)
							{
								echo "<a href='#' onClick=\"$('#svalue').val('like \'".chr($i)."%\'');$('#index').html('".chr($i)."');advanceSellerList_25012017();\">".chr($i)."</a>&nbsp;&nbsp;|&nbsp;&nbsp;";
							}
							echo "<a href='#' onClick=\"$('#svalue').val('like \'%\'');$('#index').html('A-Z');advanceSellerList_25012017();\">ALL</a>&nbsp;&nbsp;</span>";
...
<script src="http://cdn.oesmith.co.uk/morris-0.4.3.min.js"></script> <script
	...
	function advanceSellerList_25012017(){
		var selselprofilemultiple = selectmultipleuldelimiter($('#selselprofilemultiple').find('input[value]'),'selselprofilemultipleselect');
		...
		var selseller=$('#selseller').val();
		var selselprofile=selselprofilemultiple;
		var svalue=$('#svalue').val();;
